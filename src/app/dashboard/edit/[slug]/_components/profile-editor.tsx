          ...
          <div key={i} className="flex items-center gap-1.5 rounded-xl border px-2 py-1.5" style={{ borderColor: "var(--nc-border)", background: "var(--nc-surface)" }}>
            {l.platform ? (
              ...
              <select value="" onChange={(e) => setPlatform(i, e.target.value)}
                ...
                {availablePlatforms.map((p) => <option key={p} value={p}>{PLATFORM_ICONS[p] ?? "🔗"} {p}</option>)}
              </select>
            )}
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <div>
              ...
              <input value={m.emoji ?? ""} onChange={(e) => { const n = [...milestones]; n[i] = { ...n[i], emoji: e.target.value }; onChange(n); }}
                placeholder="💕" className="w-full nc-input rounded-lg px-2 py-1.5 sm:px-3 sm:py-2 text-sm" />
            </div>
          </div>
          <div>
...
export function ProfileEditor({ profile, categorySlug }: ProfileEditorProps) {
        ...
        return (
          <div>
            <textarea value={String(val ?? "")} onChange={(e) => setFieldValue(field.key, e.target.value)}
              placeholder={field.placeholder} maxLength={field.maxLength} rows={3}
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <input type="text" value={String(val ?? "#6366f1")} onChange={(e) => setFieldValue(field.key, e.target.value)}
              placeholder="#6366f1" className="nc-input flex-1 rounded-xl px-2 py-1.5 sm:px-3 sm:py-2.5 text-sm font-mono" />
          </div>
        );
      case "select":
        return (
          <select value={String(val ?? field.placeholder ?? "")} onChange={(e) => setFieldValue(field.key, e.target.value)}
            className="nc-input w-full px-2 py-1.5 sm:px-3 sm:py-2.5 text-sm">
  ...
  return (
    <div className="mx-auto max-w-3xl nc-page">
      {/* Header */}
      <div className="mb-4 sm:mb-8 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            className={`flex items-center gap-1.5 rounded-xl px-2 py-1.5 sm:px-3 sm:py-2 text-xs font-semibold transition-all sm:px-4 sm:py-2.5 sm:text-sm ${
              profile.qrLocked
          ...
          <Link href={`/${profile.slug}`} target="_blank" rel="noopener noreferrer"
            className="nc-btn-ghost flex items-center gap-1.5 rounded-xl px-2 py-1.5 sm:px-3 sm:py-2 text-xs sm:px-4 sm:text-sm">
      ...
      <div className="space-y-3">
        {sections.map((section) => {
          ...
          return (
            <div key={section.id} className="nc-card overflow-hidden rounded-xl sm:rounded-2xl">
              <button onClick={() => toggleSection(section.id)}
                className="flex w-full items-center justify-between px-3 py-2.5 sm:px-5 sm:py-4 text-left">
                <div>
              ...
              {isOpen && (
                <div className="px-3 py-2 sm:px-5 sm:py-5 space-y-2.5 sm:space-y-5">
        ...
        {/* SEO section */}
        <div className="nc-card overflow-hidden rounded-xl sm:rounded-2xl">
          ...
          {openSections.has("seo") && (
            <div className="px-3 py-2 sm:px-5 sm:py-5 space-y-2.5 sm:space-y-4">
              <div>
              ...
              <div>
                ...
                <textarea value={metaDescription} onChange={(e) => setMetaDescription(e.target.value)} maxLength={320} rows={3} placeholder="A short description for search engines and social sharing…"
                  className="nc-input w-full px-2 py-1.5 sm:px-3 sm:py-2.5 text-sm resize-none" />
              </div>
              <div>
