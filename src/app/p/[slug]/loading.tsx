...
// Skeleton shown while the QR-scanned public profile page streams in

export default function QRProfileLoading() {
  return (
    <div className="min-h-screen w-full animate-pulse bg-neutral-950 flex flex-col items-center justify-center gap-5 px-4">
      ...
      <div className="w-72 space-y-2 mt-2">
        {[1, 2].map((i) => (
          <div key={i}
            className="flex items-center gap-3 rounded-xl border border-neutral-800/60 px-4 py-2.5">
      ...
      {/* QR scan badge placeholder */}
      <div className="fixed bottom-5 left-1/2 -translate-x-1/2 h-12 w-72 rounded-2xl bg-neutral-800/60" />
    </div>
  );
}